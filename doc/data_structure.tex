...
\newcommand{\stacknotes}{
   \tabitem Last-in, first out (LIFO)&\tabitem push(), i.e., insert, to add new element to top&\tabitem O(1) for push&\tabitem O(n)\\
  &\tabitem Underflows if attempt to pop an empty stack&\tabitem pop(), i.e., delete, to remove top element&\tabitem O(1) for pop&\\
  &\tabitem Overflows if attempt to push to a full stack&\tabitem is\_empty() to check if stack is empty 
}
\begin{landscape}
